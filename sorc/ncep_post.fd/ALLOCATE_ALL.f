...
!GFS FIELD
      ! GFS ozone and oxygen fields - only needed for GFS model runs
      if (.not. minimal_memory) then
...
! add GFIP ICING - only needed when GFIP icing diagnostics are requested
      if (.not. minimal_memory) then
        allocate(icing_gfip(im,jsta_2l:jend_2u,lm))        
        allocate(icing_gfis(im,jsta_2l:jend_2u,lm))        
      endif
!
! add GTG turbulence - only needed when GTG turbulence diagnostics are requested
      if (.not. minimal_memory) then
